...
class SaleController extends Controller
{
    ...
    public function saleData(Request $request)
    {
        ...
        if (!empty($sales)) {
            foreach ($sales as $key => $sale) {
                ...
                if ($sale->service_quotation_id) {
                    $nestedData['type'] = "Service";
                }else{
                    $nestedData['type'] = "Product";
                }
                ...
                $nestedData['options'] = '<div class="btn-group">
                            ...
                            <ul class="dropdown-menu edit-options dropdown-menu-right dropdown-default" user="menu">
                                ...
                                <li><a href="' . route('sale.chalan', $sale->id) . '" class="btn btn-link"><i class="fa fa-copy"></i> ' . trans('Chalan') . '</a></li>
                                ';
                if (in_array("sales-edit", $request['all_permission'])) {
                    if ($sale->service_quotation_id != '') {
                        if ($sale->sale_status != 3)
                            $nestedData['options'] .=
                                '<li>
                            <a href="' . route('service.quotation.sale.edit', $sale->id) . '" class="btn btn-link"><i class="dripicons-document-edit"></i> ' . trans('file.edit') . '</a>
                            </li>';
                        else
    ...
    {
        $saleModalcategory = request()->session()->get('saleModalcategory');
        ...
        if ($role->hasPermissionTo('sales-add')) {
            ...
            return view('sale.create', compact('lims_customer_list', 'lims_warehouse_list', 'lims_biller_list', 'lims_pos_setting_data', 'lims_tax_list', 'interests', 'lims_priority_all', 'lims_customer_group_all', 'saleModalcategory', 'lims_categories', 'lims_category_all', 'lims_product_list', 'lims_brand_list', 'lims_category_list', 'lims_unit_list', 'productModalcategory', 'lims_product_list_without_variant', 'lims_product_list_with_variant'));
        } else
            return redirect()->back()->with('not_permitted', 'Sorry! You are not allowed to access this module');
    }
    ...
    public function store(Request $request)
    {
        ...
        $mail_data['commission_rate'] = $lims_sale_data->commission_rate;
        $mail_data['commission_amount'] = $lims_sale_data->commission_amount;
    ...
    public function posSale()
    {
        ...
        if ($role->hasPermissionTo('sales-add')) {
            ...
            $lims_biller_list = Biller::where('is_active', true)->get();
            $lims_tax_list = Tax::where('is_active', true)->get();
            $lims_product_list = Product::select('id', 'name', 'code', 'image')->whereNull('is_variant')->get();
            foreach ($lims_product_list as $key => $product) {
                $images = explode(",", $product->image);
                $product->base_image = $images[0];
            }
            $lims_product_list_with_variant = Product::select('id', 'name', 'code', 'image')->whereNotNull('is_variant')->get();
    ...
    public function saleByCsv() 
    {
        ...
        if ($role->hasPermissionTo('sales-add')) {
            ...
            return view('sale.import', compact('lims_customer_list', 'lims_warehouse_list', 'lims_tax_list'));
        } else
            return redirect()->back()->with('not_permitted', 'Sorry! You are not allowed to access this module');
    }
    ...
    public function edit($id)
    {
        ...
        if ($role->hasPermissionTo('sales-edit')) {
            ...
            return view('sale.edit', compact('lims_customer_list', 'lims_warehouse_list', 'lims_biller_list', 'lims_tax_list', 'lims_sale_data', 'lims_product_sale_data', 'lims_pos_setting_data', 'lims_customer_group_all', 'interests', 'lims_priority_all', 'lims_categories', 'lims_category_all', 'lims_product_list', 'lims_brand_list', 'lims_category_list', 'lims_unit_list', 'lims_product_list_without_variant', 'lims_product_list_with_variant'));
        } else
            return redirect()->back()->with('not_permitted', 'Sorry! You are not allowed to access this module');
    }
    ...
    public function genInvoice($id)
    {
        ...
        $invoice_no = 'S' . date("ymd") . '' . date("his");
        ...
        if ($lims_sale_data->service_quotation_id) {
            $cart = unserialize(bzdecompress(utf8_decode($lims_sale_data->service_item)));
            return view('sale.service_invoice', compact('lims_sale_data', 'lims_product_sale_data', 'lims_biller_data', 'lims_warehouse_data', 'lims_customer_data', 'lims_payment_data', 'numberInWords',
                'date', 'invoice_no','cart'));
        } else {
            return view('sale.invoice', compact('lims_sale_data', 'lims_product_sale_data', 'lims_biller_data', 'lims_warehouse_data', 'lims_customer_data', 'lims_payment_data', 'numberInWords', 'date', 'invoice_no'));
        }
    }
    ...
    public function chalan($id)
    {
        ...
        if ($lims_sale_data->service_quotation_id) {
            $cart = unserialize(bzdecompress(utf8_decode($lims_sale_data->service_item)));
            return view('sale.service_chalan', compact('lims_sale_data', 'lims_product_sale_data', 'lims_biller_data', 'lims_warehouse_data', 'lims_customer_data', 'lims_payment_data', 'numberInWords',
                'date', 'chalan_no','cart'));
        }else{
            return view('sale.chalan', compact('lims_sale_data', 'lims_product_sale_data', 'lims_biller_data', 'lims_warehouse_data', 'lims_customer_data', 'lims_payment_data', 'numberInWords', 'date', 'chalan_no'));
        }
    ...
    public function saleView($id)
    {
        ...
        $sale = Sale::find($id);
        $sale_details_list = Product_Sale::where('sale_id', $id)->get();
