...
class AccountsController extends Controller
{
    ...
    public function balanceSheet()
    {
        ...
        if($role->hasPermissionTo('balance-sheet')){
            ...
            foreach ($lims_account_list as $account) {
                ...
                $payment_recieved_from_service = ServicePayment::whereNotNull('sale_id')->where('account_id', $account->id)->sum('amount');
                ...
                $credit[] = $payment_recieved + $return_purchase + $recieved_money_via_transfer + $account->initial_balance +  $payment_recieved_from_service;
    ...
    public function accountStatement(Request $request)
    {
        ...
        if($data['type'] == '0' || $data['type'] == '2') {
            ...
            $service_credit_list = ServicePayment::whereNotNull('sale_id')->where('account_id', $data['account_id'])->whereDate('created_at', '>=' , $data['start_date'])->whereDate('created_at', '<=' , $data['end_date'])->get();
        ...
        $balance = 0;
        return view('account.account_statement', compact('lims_account_data', 'credit_list', 'debit_list', 'expense_list', 'return_list', 'purchase_return_list', 'payroll_list', 'recieved_money_transfer_list', 'sent_money_transfer_list', 'balance','service_credit_list'));
    }
