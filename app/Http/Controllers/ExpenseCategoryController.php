...
class ExpenseCategoryController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $latestCategory = ExpenseCategory::latest()->first();
        request()->session()->put('productModalcategory', $latestCategory->id);

        if(isset($request->expenseCatAdd)){
            return [
                "id"=>$latestCategory->id,
                "name"=>$latestCategory->name,
                "code"=>$latestCategory->code,
            ];
        }


        if(isset($request->returntoexpense) && $request->returntoexpense != null) {
            return back();
        } else {
