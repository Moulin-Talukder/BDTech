...
<body onload="myFunction()">
    ...
    <div class="side-navbar-wrapper">
      ...
      <div class="main-menu">
        <ul id="side-main-menu" class="side-menu list-unstyled">
          ...
          @endif
          <?php
          $service_category_permission_active = DB::table('permissions')
            ->join('role_has_permissions', 'permissions.id', '=', 'role_has_permissions.permission_id')
            ->where([
              ['permissions.name', 'service_category'],
              ['role_id', $role->id]
            ])->first();

          $service_delivery_permission_active = DB::table('permissions')
            ->join('role_has_permissions', 'permissions.id', '=', 'role_has_permissions.permission_id')
            ->where([
              ['permissions.name', 'service_delivery'],
              ['role_id', $role->id]
            ])->first();

          $print_qrcode = DB::table('permissions')->where('name', 'print_qrcode')->first();
          $print_qrcode_active = DB::table('role_has_permissions')->where([
            ['permission_id', $print_qrcode->id],
            ['role_id', $role->id]
          ])->first();

          $service_index_permission = DB::table('permissions')->where('name', 'services-index')->first();
          $service_index_permission_active = DB::table('role_has_permissions')->where([
            ['permission_id', $service_index_permission->id],
            ['role_id', $role->id]
          ])->first();

          $service_sale_index_permission = DB::table('permissions')->where('name', 'service-sales-index')->first();
          $service_sale_index_permission_active = DB::table('role_has_permissions')->where([
            ['permission_id', $service_sale_index_permission->id],
            ['role_id', $role->id]
          ])->first();

          ?>

          @if($service_category_permission_active || $service_delivery_permission_active || $service_index_permission_active || $service_sale_index_permission_active || $print_qrcode_active)
          <li><a href="#service" aria-expanded="false" data-toggle="collapse"> <i class="dripicons-list"></i><span>Service</span><span></a>
            <ul id="service" class="collapse list-unstyled ">
              @if($service_category_permission_active)
              <li id="service-category-menu"><a href="{{route('categories.index')}}">Category</a></li>
              @endif
              @if($service_index_permission_active)
              <li id="service-list-menu"><a href="{{ route('services.index') }}">Service List</a></li>
              <?php
              $service_add_permission = DB::table('permissions')->where('name', 'services-add')->first();
              $service_add_permission_active = DB::table('role_has_permissions')->where([
                ['permission_id', $service_add_permission->id],
                ['role_id', $role->id]
              ])->first();
              ?>
              @if($service_add_permission_active)
              <li id="service-create-menu"><a href="{{ route('services.create') }}">Add Service</a></li>
              @endif
              @endif

              {{--@if($service_sale_index_permission_active)--}}
              {{--<li id="service-sale-list-menu"><a href="{{ route('service.sale.list') }}">Service Sale List</a></li>--}}
              {{--<?php--}}
              {{--$services_sale_add_permission = DB::table('permissions')->where('name', 'service-sales-add')->first();--}}
              {{--$service_sale_permission_active = DB::table('role_has_permissions')->where([--}}
                {{--['permission_id', $services_sale_add_permission->id],--}}
                {{--['role_id', $role->id]--}}
              {{--])->first();--}}
              {{--?>--}}
              {{--@if($service_sale_permission_active)--}}
              {{--<li id="service-sale-menu"><a href="{{ route('service.sale') }}">Service Sale</a></li>--}}
              {{--@endif--}}
              {{--@endif--}}
              {{--@if($service_delivery_permission_active)--}}
              {{--<li id="service-delivery-menu"><a href="{{ route('service_delivery.index') }}">Service Delivery</a></li>--}}
              {{--@endif--}}
            </ul>
          </li>
          @endif
          ...
          {{--product quotation --}}
          {{--@if($index_permission_active)--}}
          {{--<li><a href="#product_quotation" aria-expanded="false" data-toggle="collapse"> <i class="dripicons-document"></i><span>{{trans('Product Quotation')}}</span><span></a>--}}
            {{--<ul id="product_quotation" class="collapse list-unstyled ">--}}
              {{--<li id="product_quotation-list-menu"><a href="{{route('quotations.index')}}">{{trans('file.Quotation List')}}</a></li>--}}
              {{--<?php--}}
          ...
          @if($index_permission_active)
          <li>
              <a href="#service_reciept" aria-expanded="false" data-toggle="collapse">
                  <i class="dripicons-document"></i>
                  <span>
                      {{trans('Service Receipt')}}
                  </span>
              </a>
            <ul id="service_reciept" class="collapse list-unstyled ">
              <li id="service_reciept-list-menu"><a href="{{route('service_quotations.index')}}">{{trans('Service Receipt List')}}</a></li>
              <?php
              $add_permission = DB::table('permissions')->where('name', 'quotes-add')->first();
              $add_permission_active = DB::table('role_has_permissions')->where([
                ['permission_id', $add_permission->id],
              ...
              ?>
              @if($add_permission_active)
              <li id="service_reciept-create-menu"><a href="{{route('service_quotations.create')}}">{{trans('Add Service Receipt')}}</a></li>
              @endif
            </ul>
          </li>
          @endif
          ...
          <li class=""><a href="#crm" aria-expanded="false" data-toggle="collapse"> <i class="dripicons-user-group"></i><span>CRM</span></a>
            <ul id="crm" class="collapse list-unstyled">
            ...
            <li id="supplier-create-menu"><a href="{{route('supplier.create')}}">{{trans('file.Add Supplier')}}</a></li>
              ...
              <li class=""><a href="#hrm" aria-expanded="false" data-toggle="collapse"> <i class="dripicons-user-group"></i><span>HRM</span></a>
                <ul id="hrm" class="collapse list-unstyled ">
                  ...
                  <li id="monthly-attendance-menu"><a href="{{route('attendance.report.momthly')}}">Monthly Attendance</a></li>

                  {{--<li id="leave-menu"><a href="{{route('leave.report.pending')}}">Leave</a>
              </li> --}}

              <li class=""><a href="#leave" aria-expanded="false" data-toggle="collapse"><span>My Profile</span></a>
                <ul id="leave" class="collapse list-unstyled ">
                  <li id="attendance-menu"><a href="{{route('attendance.my_attendance')}}">{{trans('My Attendance')}}</a></li>
                  <li id="attendance-menu"><a href="{{route('leave.index')}}">{{trans('Leaves')}}</a></li>
                </ul>
              </li>
              ...
              <li class=""><a href="#leave" aria-expanded="false" data-toggle="collapse"><span>My Profile</span></a>
                <ul id="leave" class="collapse list-unstyled ">
                  <li id="attendance-menu"><a href="{{route('attendance.my_attendance')}}">{{trans('My Attendance')}}</a></li>
                  {{-- <li id="attendance-menu"><a href="{{route('leave.index')}}">{{trans('Leaves')}}</a>
              </li> --}}
            </ul>
          </li>
        ...
        @if($reminder_index_permission_active)
        <li><a href="#reminder" aria-expanded="false" data-toggle="collapse"> <i class="fa fa-clock-o"></i><span>Reminder</span></a>
          <ul id="reminder" class="collapse list-unstyled ">
            <li id="reminder-list-menu"><a href="{{route('reminder.index')}}">Reminder List</a></li>
          </ul>
        </li>
        @endif

        <?php
        $comment_index_permission_active = DB::table('permissions')
          ->join('role_has_permissions', 'permissions.id', '=', 'role_has_permissions.permission_id')
          ->where([
            ['permissions.name', 'comments-index'],
            ['role_id', $role->id]
          ])->first();
        ?>
        @if($comment_index_permission_active)
        <li><a href="#comment" aria-expanded="false" data-toggle="collapse"> <i class="fa fa-comment"></i><span>Comment</span></a>
          <ul id="comment" class="collapse list-unstyled ">
            <li id="comment-list-menu"><a href="{{route('comment.index')}}">Comment List</a></li>
          </ul>
        </li>
        @endif
        <?php
        ...
        @endif
        <li id="customer-priority-menu"><a href="{{route('priority.index')}}">Priority</a></li>
      ...
      <div class="container-fluid">
        <div class="navbar-holder d-flex align-items-center justify-content-between">
          ...
          <ul class="nav-menu list-unstyled d-flex flex-md-row align-items-md-center">
            ...
            ?>
            @if($add_permission_active)
            <li class="nav-item"><a class="dropdown-item btn-pos btn-sm" href="{{route('sales.create')}}"><i class="dripicons-shopping-bag"></i><span> Product Sale</span></a></li>
            @endif
            <li class="nav-item"><a id="btnFullscreen"><i class="dripicons-expand"></i></a></li>
            @if(\Auth::user()->role_id <= 2) <li class="nav-item"><a href="{{route('cashRegister.index')}}" title="{{trans('file.Cash Register List')}}"><i class="dripicons-archive"></i></a></li>
              @endif

              <?php
              $reminder_permission = DB::table('permissions')->where('name', 'reminder-alert')->first();
              $reminder_permission_active = DB::table('role_has_permissions')->where([
                ['permission_id', $reminder_permission->id],
                ['role_id', $role->id]
              ])->first();

              $todayDate = date('Y-m-d');
              $todayTime = date('H:i:s');
              $endDate = date('Y-m-d', strtotime($todayDate . ' + 10 days'));


              $todayCount = DB::table('reminders')
                ->where('is_active', 1)
                ->where('date', '=', $todayDate)
                ->where('time', '>', $todayTime)
                ->count();
              $nextCount = DB::table('reminders')
                ->where('is_active', 1)
                ->where('date', '>', $todayDate)
                ->whereBetween('date', [$todayDate, $endDate])
                ->count();

              $commentTodayCount = DB::table('comments')
                ->where('created_at', '=', $todayDate)
                ->count();
              $commentNextCount = DB::table('comments')
                ->where('created_at', '>', $todayDate)
                ->whereBetween('created_at', [$todayDate, $endDate])
                ->count();
              ?>
              @if($reminder_permission_active)
              <li class="nav-item">
                <a href="{{route('reminder.index')}}" class="btn btn-link"><i class="fa fa-clock-o"></i><span class="badge badge-warning notification-number">{{ $todayCount+$nextCount }}</span></a>
                <!-- <ul class="dropdown-menu edit-options dropdown-menu-left dropdown-default notifications" user="menu" style="right:20px !important">
                  <li class="reminder">
                    <a href="/reminder" class="btn btn-link"> You have {{ $todayCount + $nextCount }} Reminders</a>
                  </li>

                  <hr>
                  <li class="reminder">
                    <a href="" class="btn btn-link" style="padding:0px 10px; color:rgb(190, 7, 7);"><i class="dripicons-user" style="color:rgb(190, 7, 7);"></i></a>
                    {{--<a href="" class="btn btn-link" style="padding:0px 10px; color:rgb(190, 7, 7);"><i class="fa fa-calendar" style="color:rgb(190, 7, 7);"></i>{{ date('d-m-Y') }} || {{ date('h:i A') }}</a>--}}
                  </li>
              ...
              <li class="nav-item" id="notification-icon">
                ...
                <a href="{{route('comment.index')}}" class="btn btn-link"><i class="dripicons-bell"></i><span class="badge badge-danger notification-number">{{ $commentTodayCount+$commentNextCount }}</span></a>
  ...
  <div class="page">
    ...
    <div id="expense-modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true" class="modal fade text-left">
              ...
              <div class="col-md-6">
                <div class="form-group">
                  ...
                  <div class="input-group">
                    <select id="expenseCat" name="expense_category_id" class="selectpicker form-control sel" required data-live-search="true" data-live-search-style="begins" title="Select Expense Category...">
                      @foreach($lims_expense_category_list as $expense_category)
                      <option
                          {{--@isset($productModalcategory) @if($productModalcategory==$expense_category->id) selected--}}
                        {{--@endif--}}
                        {{--@endisset --}}
                    ...
                    <div class="input-group-append">
                      <button id="" type="button" class="btn btn-sm" data-toggle="modal" data-target="#createExpenseModal" title=""><i class="fa fa-plus"></i>
                      </button>
                    </div>
                  </div>
      ...
      <div id="createExpenseModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true" class="modal fade text-left">
          <div role="document" class="modal-dialog">
              <div class="modal-content">
                  {{--{!! Form::open(['route' => 'expense_categories.store', 'method' => 'post', 'files' => true]) !!}--}}
                  <form id="expense-category-form" action="">
                      <div class="modal-header">
                          <h5 id="exampleModalLabel" class="modal-title">{{trans('file.Add Expense Category')}}</h5>
                          <button type="button" data-dismiss="modal" aria-label="Close" id="expenseCls" class="close"><span aria-hidden="true"><i class="dripicons-cross"></i></span></button>
                      </div>
                      <div class="modal-body">
                          <p class="italic"><small>{{trans('file.The field labels marked with * are required input fields')}}.</small></p>
                          <div class="form-group">
                              ...
                              <div class="input-group">
                                  ...
                                  <div class="input-group-append">
                                      <button id="genbuttonCatExpense" type="button" class="btn btn-default">{{trans('file.Generate')}}</button>
                                  </div>
                              </div>
                          </div>
                          <input type="hidden" name="expenseCatAdd" value="1">
                          <div class="form-group">
                              <label>{{trans('file.name')}} *</label>
                              {{Form::text('name',null,array('required' => 'required', 'class' => 'form-control', 'placeholder' => 'Type expense category name...'))}}
                          </div>
                          <input type="hidden" name="is_active" value="1">
                          <input type="hidden" name="returntoexpense" value="1">
                          <div class="form-group">
                              <input type="submit" id="expenseCatAdd" value="{{trans('file.submit')}}" class="btn btn-primary">
                          </div>
                      </div>
                  </form>
                  {{--{{ Form::close() }}--}}
  ...
  <script type="text/javascript">
    ...
    $('#genbuttonCatExpense').on("click", function(){
    ...
    $('#expenseCatAdd').on("click", function (e) {
        e.preventDefault();
        $.ajax({
            type:'POST',
            //headers: {'X-CSRF-TOKEN': $('meta[name="csrf_token"]').attr('content')},
            url:"{{route('expense_categories.store')}}",
            data: $("#expense-category-form").serialize(),
            success:function(response){
                console.log(response);
                //location.href = '../purchases/create';
                var val = response.name + ' (' + response.code + ')';
                $('#expenseCat').append($('<option>', {
                    value: response.id,
                    text : val
                }));
                $("#expenseCat").val(response.id).change();
                $('#expenseCls').click();
